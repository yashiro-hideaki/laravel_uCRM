...
<template>
            ...
            <table class="table-auto w-full text-left whitespace-no-wrap">
              ...
              <tbody>
                <tr v-for="customer in customers.value.data" :key="customer.id">
                  <td class="px-4 py-3">
                    <!-- php artisan route:list で確認すると、showはcustomers/{customer}となっているため、渡すパラメーターはcustomer:customer.idとなる -->
                    <button type="button" @click="setCustomer({
                      id: customer.id, kana:
                      customer.kana
                    })" >
                      {{ customer.id }}
  ...
  <button class="flex mx-auto text-white bg-teal-500 border-0 py-2 px-8 focus:outline-none hover:bg-teal-600 rounded text-lg" @click="searchCustomers" type="button" data-micromodal-trigger="modal-1">検索する</button>
</template>