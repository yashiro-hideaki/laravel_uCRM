...
const form = reactive({
  //controllerから渡ってきた値を変数propsにして、リアクティブに渡す
  id:props.item.id,
  name:props.item.name,
  memo:props.item.memo,
  price:props.item.price,
  is_selling:props.item.os_selling
})
//引数に指定した値は、reactiveから渡ってくる
const updateItem = (id) => {
  //php artisan route:listで見ると、updateはput/patchであるため、post通信ではなくputになる
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900">
                      <section class="text-gray-600 body-font relative">
                        <Validation class="mb-4"/>
                        <!-- 引数で指定した値がreactiveのidに渡る -->
                              ...
                              <div class="flex flex-wrap -m-2">
                                ...
                                <div class="p-2 w-full">
                                  <div class="relative">
                                    ...
                                    <label for="" class="mx-4">販売中</label>
                                    <input type="radio" id="is_selling" name="is_selling" v-model="form.is_selling" value="0">
                                    <label for="" class="mx-4">停止中</label>
                                  </div>
                                </div>
                                <div class="p-2 w-full">
                                  <button class="flex mx-auto text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg">商品更新</button>
                                </div>
                              </div>
                            </div>
                          </div>
